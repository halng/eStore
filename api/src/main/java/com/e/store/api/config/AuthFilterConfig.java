...
public class AuthFilterConfig extends AbstractGatewayFilterFactory<AuthFilterConfig.Config> {
	...
	public GatewayFilter apply(Config config) {
		...
		return (exchange, chain) -> {
			...
			LOG.info(
					"Incoming request %s is routed to id: %s, uri: %s".formatted(originalUri, route.getId(), routeUri));

			if (ExcludeUrlConfig.isSecure(path, method)) {
				String bearerToken = Objects.requireNonNull(request.getHeaders().get("authorization")).get(0);
