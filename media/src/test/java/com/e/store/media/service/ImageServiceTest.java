...
public class ImageServiceTest {
	...
	void getImages() {
		...
		Image expectedImage1 = Image.builder().id("xxx").caption("x-caption").data("x".getBytes()).fileType("jpeg")
			.build();
		Image expectedImage2 = Image.builder().id("yyy").caption("y-caption").data("y".getBytes()).fileType("jpeg")
			.build();

		when(this.iImageRepository.findAllById(imagesReqVm.imageIds())).thenReturn(
			Arrays.asList(expectedImage1, expectedImage2));
	...
	void setImage_shouldThrowException_whenIdValid() {
		Image expectedImage = Image.builder().id("yyy").caption("y-caption").data("y".getBytes()).fileType("jpeg")
			.build();
