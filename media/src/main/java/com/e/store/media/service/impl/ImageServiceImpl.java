...
public class ImageServiceImpl implements IImageService {
    ...
    public ResponseEntity<ResVm> uploadImage(List<MultipartFile> files, String caption) {
        ...
        LOG.info(
            String.format("Receive request upload image from %s - total %d file(s) - caption: %s", user, files.size(),
                caption));

        for (MultipartFile file : files) {
            try {
                String finalCaption = caption + "_" + file.getOriginalFilename();
                Image image = Image.builder().caption(finalCaption).data(file.getBytes())
    ...
    public ResponseEntity<List<ImagesResVm>> getImages(ImagesReqVm imagesReqVm) {
        ...
        for (Image img : imageList) {
            String downloadUrl = ServletUriComponentsBuilder
                .fromHttpUrl("http://localhost:9090/api/v1/media")
                .path("/image/")
                .path(img.getId())
                .toUriString();

            images.add(new ImagesResVm(img.getCaption(), img.getFileType(), downloadUrl, (long) img.getData().length));
        }
    ...
    public ResponseEntity<byte[]> getImage(String id) {
        ...
        Image image = this.iImageRepository.findById(id)
