...
public class AuthMessageMethodTest {
  ...
  public void fromJsonObject_shouldThrowException_WhenTimeOut() {
    ...
    ActiveAccountTimeOutException exception =
        Assert.assertThrows(
            ActiveAccountTimeOutException.class,
            () -> {
              AuthMessageVm.fromJsonObject(object);
            });
  ...
  public void getUrlActiveAccountTest() {
    ...
    assertEquals(
        authMessageVm.getUrlConfirm(),
        String.format("/active-account?token=%s&email=%s", "123-344", "email"));
  }
}
