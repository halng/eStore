    ...
    public static AuthMessageVm fromJsonObject(JsonObject json) {
        if (json.has("expiryDate")
            ...
            if (timeExpire.isBefore(maxTime)) {
                throw new ActiveAccountTimeOutException(
                    "Send email active account failed. Time out expiryDate");
            }
            return new AuthMessageVm(
                json.get("email").getAsString(),
                json.get("username").getAsString(),
                json.get("activeToken").getAsString(),
                timeExpire.getEpochSecond());
        }
        throw new IllegalArgumentException(
            "Message don't have enough data. Miss expiryDate field.");
    }
