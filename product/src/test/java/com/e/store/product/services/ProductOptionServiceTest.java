...
public class ProductOptionServiceTest {
  ...
  void setup() {
    ...
    optionCreateReqVm =
        new ProductOptionCreateReqVm("Test option name", "test description", "text");
  ...
  void createNewProductOption_shouldThrowException_whenDataInvalid() {
    when(this.iProductOptionRepository.existsByNameAndCreateBy(anyString(), anyString()))
        .thenReturn(true);
    ...
    Assertions.assertEquals(
        "Product Option with name Test option name already exists!",
  ...
  void createNewProductOption_shouldSuccess_whenDataValid() {
    ...
    Assertions.assertEquals(
        "Create new option successfully - id: xxx-111-222", res.getBody().message());
  }
  ...
  void getAllOptionTest() {
        ...
        new Page<ProductOption>() {
          ...
          public List<ProductOption> getContent() {
            ProductOption productOption =
                ProductOption.builder().displayType("text").name("test").description("hh").build();
    ...
    when(this.iProductOptionRepository.findByCreatorWithPagination(any(), any()))
        .thenReturn(productOptionPage);

    ResponseEntity<PagingResVm<ProductOptionResVm>> res = this.productOptionService.getAllOption(1);

    Assertions.assertNotNull(res);
    Assertions.assertNotNull(res.getBody());
    Assertions.assertEquals(1, res.getBody().totalPages());
    Assertions.assertEquals(2L, res.getBody().totalItems());
  }
  ...
  void updateOptionTest() {
    ...
    ResponseEntity<ResVm> response =
        this.productOptionService.updateOption("iii", optionCreateReqVm);

    Assertions.assertNotNull(response);
    Assertions.assertNotNull(response.getBody());
    Assertions.assertEquals(
        "Update option Test option name successfully", response.getBody().message());
  }
  ...
  void deleteOption_shouldSuccess_whenDataValid() {
    ProductOption productOption =
        ProductOption.builder()
            .id("iii")
            .name("test-name")
            .productOptionValueList(List.of())
            .build();
    when(this.iProductOptionRepository.findById(any())).thenReturn(Optional.of(productOption));
