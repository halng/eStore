...
public class ProductAttributeServiceTest {
    ...
    void setup() {
        ...
        productAttribute = ProductAttribute.builder().id("111").description("this is description").name("test 1").status(
    ...
    void getById_shouldThrowException_whenEntityNotExist(){
        ...
        Assertions.assertEquals(String.format("Product Attribute with id: %s does not exist! Pls try again!", "111"), exception.getMessage());
    }
    ...
    void getAllProductAttributeTest(){
        ...
        Page<ProductAttribute> productAttributes = new Page<ProductAttribute>() {
            @Override
    ...
    void updateAttributeTest(){
        when(this.iProductAttributeRepository.findById(anyString())).thenReturn(Optional.of(productAttribute));

        ResponseEntity<ResVm> response = this.iProductAttributeService.updateAttribute("111", productAttributeCreateReqVm);
        Assertions.assertNotNull(response);
        Assertions.assertNotNull(response.getBody());
    }
    ...
    void updateStatusAttribute(){
        ...
        ResponseEntity<ResVm> response2 = this.iProductAttributeService.updateStatusAtt("111", "disable");
